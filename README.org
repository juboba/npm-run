...
* Description
This package is minimal and only centers around running 
scripts in node projects (shines in monorepos). You can use ~npm run~,
but also ~yarn~, ~pnpm run~ or anything else.

There are more complete solutions (see [[#related-packages][Related packages]]) that
provide full interfaces to the npm cli.
...
#+begin_src elisp
(setq npm-run-run-command "yarn")  ; Use yarn instead of 'npm run'
#+end_src
...
Oher packages that provide a full interface to npm:
- [[https://github.com/shaneikennedy/npm.el][GitHub -
  shaneikennedy/npm.el: NPM client for emacs]] Uses transient
- [[https://github.com/mojochao/npm-mode][GitHub - mojochao/npm-mode: An Emacs minor mode for working with NPM
  projects.]]  Keybind driven 
